 ...
 */
static void prvRestoreContextOfFirstTask(void) __attribute__((
...
static void prvSVCHandler(uint32_t* pulRegisters) __attribute__((
            noinline)) PRIVILEGED_FUNCTION;

/*
 * Prototypes for all the MPU wrappers.
 */
BaseType_t MPU_xTaskGenericCreate(TaskFunction_t pvTaskCode,
                                  const char* const pcName, uint16_t usStackDepth, void* pvParameters,
                                  UBaseType_t uxPriority, TaskHandle_t* pxCreatedTask,
                                  StackType_t* puxStackBuffer, const MemoryRegion_t* const xRegions);
...
UBaseType_t MPU_uxTaskGetSystemState(TaskStatus_t* pxTaskStatusArray,
                                     UBaseType_t uxArraySize, uint32_t* pulTotalRunTime);
...
BaseType_t MPU_xQueueGenericReceive(QueueHandle_t pxQueue, void* const pvBuffer,
...
BaseType_t MPU_xQueueAltGenericSend(QueueHandle_t pxQueue,
                                    const void* const pvItemToQueue, TickType_t xTicksToWait,
                                    BaseType_t xCopyPosition);
BaseType_t MPU_xQueueAltGenericReceive(QueueHandle_t pxQueue,
                                       void* const pvBuffer, TickType_t xTicksToWait, BaseType_t xJustPeeking);
...
static void prvSetupMPU(void)
{
    ...
    if (*portMPU_TYPE == portEXPECTED_MPU_TYPE_VALUE) {
        ...
        *portMPU_REGION_BASE_ADDRESS = ((uint32_t) __FLASH_segment_start__)
                                       |      /* Base address. */
        ...
        *portMPU_REGION_ATTRIBUTE = (portMPU_REGION_READ_WRITE |
                               ...
                               uint16_t usStackDepth)
{
    ...
    if (xRegions == NULL) {
        ...
        for (ul = 2; ul <= portNUM_CONFIGURABLE_REGIONS; ul++) {
            xMPUSettings->xRegion[ ul ].ulRegionBaseAddress = (portSTACK_REGION + ul) |
                    portMPU_REGION_VALID;
        ...
        if (usStackDepth > 0) {
            ...
            xMPUSettings->xRegion[ 0 ].ulRegionAttribute =
                ...
                (prvGetMPURegionSizeSetting((uint32_t) usStackDepth * (uint32_t) sizeof(
                                                StackType_t))) |
                (portMPU_REGION_CACHEABLE_BUFFERABLE) |
                (portMPU_REGION_ENABLE);
        }
                                  ...
                                  StackType_t* puxStackBuffer, const MemoryRegion_t* const xRegions)
{
    ...
    xReturn = xTaskGenericCreate(pvTaskCode, pcName, usStackDepth, pvParameters,
                                 uxPriority, pxCreatedTask, puxStackBuffer, xRegions);
    portRESET_PRIVILEGE(xRunningPrivileged);
    return xReturn;
}
/*-----------------------------------------------------------*/
                                     ...
                                     UBaseType_t uxArraySize, uint32_t* pulTotalRunTime)
{
    ...
    uxReturn = uxTaskGetSystemState(pxTaskStatusArray, uxArraySize,
                                    pulTotalRunTime);
    portRESET_PRIVILEGE(xRunningPrivileged);
    return xReturn;
}
                                    ...
                                    BaseType_t xCopyPosition)
{
    ...
    xReturn =   BaseType_t xQueueAltGenericSend(pxQueue, pvItemToQueue,
                xTicksToWait, xCopyPosition);
    portRESET_PRIVILEGE(xRunningPrivileged);
    return xReturn;
}
#endif
                                       ...
                                       void* const pvBuffer, TickType_t xTicksToWait, BaseType_t xJustPeeking)
{
    ...
    xReturn = xQueueAltGenericReceive(pxQueue, pvBuffer, xTicksToWait,
                                      xJustPeeking);
    portRESET_PRIVILEGE(xRunningPrivileged);
    return xReturn;
}
