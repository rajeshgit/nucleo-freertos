...
static const uint32_t ulMatchValueForOneTick = ((configPERIPHERAL_CLOCK_HZ /
        portCLOCK_DIVISOR) / configTICK_RATE_HZ);
...
static const TickType_t xMaximumPossibleSuppressedTicks = USHRT_MAX / ((
            configPERIPHERAL_CLOCK_HZ / portCLOCK_DIVISOR) / configTICK_RATE_HZ);
...
static const uint32_t ulStoppedTimerCompensation = 100UL /
        (configCPU_CLOCK_HZ / (configPERIPHERAL_CLOCK_HZ / portCLOCK_DIVISOR));
...
void vPortSuppressTicksAndSleep(TickType_t xExpectedIdleTime)
{
    ...
    if (eSleepAction == eAbortSleep) {
        ...
        if (ulTickFlag != pdFALSE) {
            ...
            period remains. */
            ulMatchValue = ulCurrentCount - (ulCompleteTickPeriods *
