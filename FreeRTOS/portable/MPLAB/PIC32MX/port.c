 ...
 */
extern void __attribute__((interrupt(ipl1),
                           vector(configTICK_INTERRUPT_VECTOR))) vPortTickInterruptHandler(void);
...
void __attribute__((interrupt(ipl1),
                    vector(_CORE_SOFTWARE_0_VECTOR))) vPortYieldISR(void);
...
{
    const uint32_t ulCompareMatch = ((configPERIPHERAL_CLOCK_HZ /
                                      portTIMER_PRESCALE) / configTICK_RATE_HZ) - 1;
